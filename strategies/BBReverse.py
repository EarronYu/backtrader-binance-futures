...
class BBReverse(bt.Strategy):
    ...
    """
    params = dict(bollwindow=200, devfactor=2, bias_pct=0.05, pgoperiod=200, pgorange=3.0)
    ...
    def __init__(self):
        ...
        pgo = bt.indicators.PrettyGoodOscillator(self.data, period=self.params.pgoperiod, plot=False)
        # low_pgo = bt.indicators.If(abs(pgo) < self.params.pgorange, 1, 0)
    ...
    def stop(self):
        # 新增加一个stop策略完成函数
        # 用于输出执行后数据
        self.log(f'(策略参数 bollwindow={self.params.bollwindow}, devfactor={self.params.devfactor}, bias_pct={self.params.bias_pct}, pgoperiod={self.params.pgoperiod}, pgorange={self.params.pgorange}) ，最终资产总值：{self.broker.getvalue()}', fgPrint=True)
